...
We can connect these two points with an arc $c_\alpha$ in $(H_1 - B_{24}) \cap H_2 = M_x \cap X_1 \cap X_2$ and an arc $c_\beta$ in $B_{24} \cap (H_1 - B_{24}) = M_x \cap X_1 \cap X_3$.
The union $c_\alpha \cup c_\beta$ is a simple closed curve in $\del (H_1 - B_{24})$, and this curve bounds a disk $D_i$ in $H_1 - B_{24}$.    See Figure \ref{getU} for reference.
Indeed, $D_{\alpha_i} =  X_1 \cap X_2$ can be obtained by taking the union $\bigcup_{x \in I_{24}} c_\alpha$.
$D_{\beta}$ is slightly more complicated, consisting of the union of $\bigcup_{x \in I_{24}} c_\beta$ together with copies of $D_i$ in each of $M_{2/5}$ and $M_{3/5}$.
