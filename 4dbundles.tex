...
The reader may have observed that the location the punctures are drawn is not completely free.  Indeed, for any two punctures that are tubed together, drawing one of the two determines where we must draw the other.  So for example the punctures of $\Sigma_1$ and $\Sigma_2$ that are tubed together should be drawn so that they are identified by $\eta_{1 \to 2}$.  Now that $\eta_{1 \to 3}$ has been described, we can apply this to the punctures of $\Sigma_1$ and $\Sigma_3$ that are tubed together as well.
...
Then $D_{\alpha_i} \cup D_{\beta_i}$ is a (nonseparating) 2--sphere lying in $\del X_1$, and therefore bounds a (nonseparating) 3--ball $B_i$ in $X_1$ by \cite{LaudenbachPoenaru1}.
...
Consider a fixed value of $i$ such that $1 \le i \le g$, so $\alpha_i$ and $\beta_i$ are parallel and are both dual to $\gamma_i$.  Let $x \in \mathring I_{24}$ and look at a single slice $M_x = \{x\} \times M$.
...
We can connect these two points with an arc $c_{\alpha_i}$ in $(H_1 - B_{24}) \cap H_2 = M_x \cap X_1 \cap X_2$ and an arc $c_{\beta_i}$ in $B_{24} \cap (H_1 - B_{24}) = M_x \cap X_1 \cap X_3$.
The union $c_{\alpha_i} \cup c_{\beta_i}$ is a simple closed curve in $\del (H_1 - B_{24})$, and this curve bounds a disk $D_i$ in $H_1 - B_{24}$.    See Figure \ref{getU} for reference.
Indeed, $D_{\alpha_i} =  X_1 \cap X_2$ can be obtained by taking the union $\bigcup_{x \in I_{24}} c_{\alpha_i}$.
$D_{\beta_i}$ is slightly more complicated, consisting of the union of $\bigcup_{x \in I_{24}} c_{\beta_i}$ together with copies of $D_i$ in each of $M_{2/5}$ and $M_{3/5}$.

The product $B_i = I_{24} \times D_i$ is the desired ball giving a destabilization.  Hence, the destabilization is achieved by setting
...
There are $g$ such destabilizations, so to get the final result we must find disjoint $D_1 \cdots D_g$ such that $B_i = I_{24} \times D_i$, and set
\begin{align*}
...
We can then set  $U=X_1' \cap M_x$ and $V = X_3' \cap M_x$.  The boundary $U \cap V$ consists of four disks, two of which are shown in Figure \ref{fig_uvboundary}.  Figure \ref{fig_splittinguv} shows that by performing an isotopy, this is in fact the same split $U \cap V$ as in Figure \ref{fig_disjointballs}.
