...
%DIF DEL 4dbundlesinitial.tex   Sat Apr 18 23:39:58 2020
%DIF ADD 4dbundles.tex          Sun May  3 18:26:26 2020
...
A \emph{trisection} of a closed, smooth, \DIFdelbegin \DIFdel{four dimensional }\DIFdelend \DIFaddbegin \DIFadd{four--dimensional }\DIFaddend manifold $X$ is a decomposition $X = X_1 \cup X_2 \cup X_3$ such that each $X_i$ is a 4--dimensional handlebody $\natural^{k_i} S^1 \times B^3$ and each pairwise intersection $X_i \cap X_j$, $i \not = j$ is a 3--dimensional handlebody $\natural^g S^1 \times D^2$.
...
Since each $X_i$ is a \DIFdelbegin \DIFdel{four dimensional }\DIFdelend \DIFaddbegin \DIFadd{four--dimensional }\DIFaddend handlebody $\natural^{k_i} S^1 \times B^3$, $\del X_i$ is diffeomorphic to $\#^{k_i} S^1 \times S^2$.
...
There are three genus 1 trisections of $S^4$ up to \DIFdelbegin \DIFdel{isotopy }\DIFdelend \DIFaddbegin \DIFadd{diffeomorphism }\DIFaddend (see Figure \ref{s3g1}), and thus three possible unbalanced stabilizations.
...
$B_{13}$, $B_{23a}$, and $B_{23b}$ are chosen to be disjoint balls in $H_1 \cup H_2$, each intersecting $\Sigma$ in a disk, such that $B_{23a}$ and $B_{23b}$ intersect \DIFdelbeginFL \DIFdelFL{$V$ while $B_{13}$ intersects }\DIFdelendFL $U$.}
...
We further require that $B_{23a} \cap H_1$ and $B_{23b} \cap H_1$ lie in $U$ \DIFdelbegin \DIFdel{, while $B_{13} \cap H_1$ lies in $V$ }\DIFdelend (see Figure \ref{fig_disjointballs}).
...
Recall that this rectangle is the image of a map $F\colon I \times M \to I \times I$, where $F(t,x) = (t,h(x))$ with $h\colon M \to I$ a self indexing \DIFdelbegin \DIFdel{mose }\DIFdelend \DIFaddbegin \DIFadd{Morse }\DIFaddend function on $M$.
...
\DIFaddbegin \DIFadd{The intersection }\DIFaddend $R_1 \cap R_2 \cap R_3$ consists of two points in the figure, which pull back under $F$ to two copies of $\Sigma$.
We call the pullback of the left point \DIFdelbegin \DIFdel{of $R_1 \cap R_2 \cap R_3$ }\DIFdelend $\Sigma_1$ \DIFdelbegin \DIFdel{, }\DIFdelend and the pullback of the right point $\Sigma_2$.
...
The $B_{12} \cap \Sigma_i$ punctures of the two surfaces are connected by the open tube in $I_{12} \times M$.
 The part of the triple intersection \DIFdelbegin \DIFdel{in lying }\DIFdelend \DIFaddbegin \DIFadd{lying in }\DIFaddend the pullback of the figure therefore is a twice punctured genus-$2g$ surface as seen in Figure \ref{fig_coresubsurface}.
...
\caption{A restricted section of the \DIFdelbeginFL \DIFdelFL{spine}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{trisection surface}\DIFaddendFL .}
...
The trisection surface $X_1 \cap X_2 \cap X_3$ then consists of three copies of $\Sigma$ connected \DIFdelbegin \DIFdel{up }\DIFdelend by three tubes.
...
\DIFdelbegin \DIFdel{Note that we can choose the }\DIFdelend \DIFaddbegin \DIFadd{The reader may have observed that the location the punctures are drawn is not completely free.  Indeed, for any two punctures that are tubed together, drawing one of the two determines where we must draw the other.  So for example the }\DIFaddend punctures of $\Sigma_1$ and \DIFdelbegin \DIFdel{$\Sigma_3$ }\DIFdelend \DIFaddbegin \DIFadd{$\Sigma_2$ }\DIFaddend that are tubed together \DIFdelbegin \DIFdel{in a way that ensures that $\eta_{3 \to 1}$ identifies them.
Indeed $\eta_{3 \to 1}$ is induced by $I_{23} \times M$, so if we choose a generic point $x \in \Sigma$, and choose the endpoints of $I_{23} \times \{x\}$ to be the punctures in $\Sigma_3$ and }\DIFdelend \DIFaddbegin \DIFadd{should be drawn so that they are identified by $\eta_{1 \to 2}$.  Now that $\eta_{1 \to 3}$ has been described, we can apply this to the punctures of }\DIFaddend $\Sigma_1$ \DIFdelbegin \DIFdel{, then they are identified by $\eta_{3 \to 1}$ as desired}\DIFdelend \DIFaddbegin \DIFadd{and $\Sigma_3$ that are tubed together as well}\DIFaddend .
...
The \DIFdelbeginFL \DIFdelFL{red }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{orange }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{purple }\DIFaddendFL oriented curves in each copy of $\Sigma$ therefore are the same pair of curves slid along $I \times \Sigma$.
Passing through the gluing monodromy, however, will send \DIFdelbeginFL \DIFdelFL{green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{orange }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{red }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{purple }\DIFaddendFL curve to something else in general.}
...
\DIFaddbegin \DIFadd{We describe }\DIFaddend $\alpha$ curves \DIFdelbegin \DIFdel{denote }\DIFdelend \DIFaddbegin \DIFadd{determining }\DIFaddend a cut system for $X_1 \cap X_2$, $\beta$ \DIFaddbegin \DIFadd{curves a cut system }\DIFaddend for $X_1 \cap X_3$, and $\gamma$ \DIFaddbegin \DIFadd{curves a cut system }\DIFaddend for $X_2 \cap X_3$.
...
We begin by constructing \DIFdelbegin \DIFdel{the genus }\DIFdelend \DIFaddbegin \DIFadd{an unbalanced }\DIFaddend $4g+1$ unbalanced trisection \DIFdelbegin \DIFdel{shown in Figure \ref{unbalancedbreakdown} }\DIFdelend \DIFaddbegin \DIFadd{from Figure \ref{preservebreakdown} }\DIFaddend by attaching tubes through the areas indicated by horizontal lines as in the previous constructions.  After constructing a diagram for this \DIFdelbegin \DIFdel{trisection }\DIFdelend \DIFaddbegin \DIFadd{unbalanced trisection, }\DIFaddend we will find a set of $g$ destabilizations resulting in a genus $3g+1$ diagram.  We \DIFaddbegin \DIFadd{will }\DIFaddend conclude by showing that the destabilized \DIFdelbegin \DIFdel{trisection is }\DIFdelend \DIFaddbegin \DIFadd{diagram corresponds to }\DIFaddend the same as the trisection \DIFaddbegin \DIFadd{as was }\DIFaddend constructed in Section \ref{sec_mainthm}.

\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[height=1.8in]{MxS1_unbalanced.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{$I \times M$ is depicted as a rectangle with $I$ as the horizontal axis and $M$ as the vertical axis.
The middle horizontal line represents $I \times \Sigma$, so the copy of $M$ represented by each vertical slice is cut by the middle line into copies of the handlebodies $H_1$ and $H_2$.}}
%DIFAUXCMD
%DIFDELCMD < \label{unbalancedbreakdown}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The construction will mainly differ in the handling of $R_4$, as we will not initially split $R_4$ into two regions as we did when constructing a balanced trisection.  As in Section \ref{sec_mainthm}, define $I_{13}, I_{23a}, I_{23b}$, and $I_{24}$ such that $I_{ij}$ corresponds to an interval where $R_i$ and $R_j$ meet in a horizontal segment.  Define small disjoint balls $B_{13}, B_{23a}, B_{23b}$, and $B_{24}$ in $M$, each meeting $\Sigma$ in a single disk.  Than we obtain a genus $4g+1$ unbalanced trisection by defining
}\DIFaddend 

\DIFaddbegin \begin{align*}
&\DIFadd{X_1 = (R_1 \cup R_4 \cup N_{23a} \cup N_{23b}) - (\mathring N_{13} \cup \mathring N_{24}) }\\
&\DIFadd{X_2 = (R_2 \cup N_{13}) - (\mathring N_{23a} \cup \mathring N_{23b} \cup \mathring N_{24}) }\\
&\DIFadd{X_3 = (R_3 \cup N_{24}) - (\mathring N_{13} \cup \mathring N_{23a} \cup \mathring N_{23b})
}\end{align*}

\DIFadd{Here, $X_1$ has genus $2g + 1$, while $X_2$ and $X_3$ have genus $g+1$.
}

\DIFadd{We can now begin constructing a diagram for this trisection.  }\DIFaddend To begin, draw the triple intersection surface for this diagram as four twice punctured copies of $\Sigma$ connected by tubes.
...
\DIFdelbegin \DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{Suppose }\DIFaddend $D_{\alpha_i}$ be a meridian disk of $X_1 \cap X_2$ with boundary $\alpha_i$, and $D_{\beta_i}$ a meridian disk of $X_1 \cap X_3$ with boundary $\beta_i$.
\DIFaddbegin \DIFadd{Then }\DIFaddend $D_{\alpha_i} \cup D_{\beta_i}$ is a \DIFaddbegin \DIFadd{(nonseparating) }\DIFaddend 2--sphere lying in $\del X_1$, and therefore bounds a \DIFaddbegin \DIFadd{(nonseparating) }\DIFaddend 3--ball $B_i$ in $X_1$ by \cite{LaudenbachPoenaru1}.
Hence \DIFdelbegin \DIFdel{, }\DIFdelend each $D_{\alpha_i} \cup D_{\beta_i}$ is the belt sphere of a 1--handle in a handle decomposition of $X_1$.
...
Each compression decreases the genus of $X_1$ by 1.  \DIFaddbegin \DIFadd{We will find destabilizations by defining such pairs of disks $D_{\alpha_i}$ and $D_{\beta_i}$.
}\DIFaddend 

\DIFdelbegin \DIFdel{Let $x \in \mathring I_{12}$ }\DIFdelend \DIFaddbegin \DIFadd{Consider a fixed value of $i$ such that $1 \le i \le g$, so $\alpha_i$ }\DIFaddend and \DIFaddbegin \DIFadd{$\beta_i$ are parallel and are both dual to $\gamma_i$.  Let $x \in \mathring I_{24}$ and }\DIFaddend look at a single slice $M_x = \{x\} \times M$.
Then \DIFdelbegin \DIFdel{$M_x \cap X_1 = H_1 - B_{12}$, $M_x \cap X_2 = H_2 - B_{12}$, and $M_x \cap X_3 = B_{12}$}\DIFdelend \DIFaddbegin \DIFadd{$M_x \cap X_1 = H_1 - B_{24}$, $M_x \cap X_2 = H_2 - B_{24}$, and $M_x \cap X_3 = B_{24}$}\DIFaddend .
$M_x \cap \alpha_i = M_x \cap \beta_i$ is a pair of two points.
We can connect these two points with an arc \DIFdelbegin \DIFdel{of $D_{\alpha_i}$ in $(H_1 - B_{12}) \cap H_2 = M_x \cap X_1 \cap X_2$ }\DIFdelend \DIFaddbegin \DIFadd{$c_{\alpha_i}$ in $(H_1 - B_{24}) \cap H_2 = M_x \cap X_1 \cap X_2$ }\DIFaddend and an arc \DIFdelbegin \DIFdel{of $D_{\beta_i}$ in $B_{12} \cap (H_1 - B_{12}) = M_x \cap X_1 \cap X_3$.
...
%DIFDELCMD < %%%
\DIFdel{We now need to find the ball $B_i \subset X_1$ bounded by $D_{\alpha_i} \cup D_{\alpha_i}$.
Looking back at a single $M_x$, the union of the arc from $D_{\alpha_i}$ and the arc from $D_{\beta_i}$ }\DIFdelend \DIFaddbegin \DIFadd{$c_{\beta_i}$ in $B_{24} \cap (H_1 - B_{24}) = M_x \cap X_1 \cap X_3$.
The union $c_{\alpha_i} \cup c_{\beta_i}$ }\DIFaddend is a simple closed curve in \DIFdelbegin \DIFdel{$\del (H_1 - B_{12})$}\DIFdelend \DIFaddbegin \DIFadd{$\del (H_1 - B_{24})$}\DIFaddend , and this curve bounds a disk $D_i$ in \DIFdelbegin \DIFdel{$H_1 - B_{12}$}\DIFdelend \DIFaddbegin \DIFadd{$H_1 - B_{24}$}\DIFaddend .    See Figure \ref{getU} \DIFaddbegin \DIFadd{for reference.
Indeed, $D_{\alpha_i} =  X_1 \cap X_2$ can be obtained by taking the union $\bigcup_{x \in I_{24}} c_{\alpha_i}$}\DIFaddend .
\DIFdelbegin \DIFdel{It follows that the ball $B_i$ is in fact $I_{12} \times D_i$, so }\DIFdelend \DIFaddbegin \DIFadd{$D_{\beta_i}$ is slightly more complicated, consisting of the union of $\bigcup_{x \in I_{24}} c_{\beta_i}$ together with copies of $D_i$ in each of $M_{2/5}$ and $M_{3/5}$.
}

\DIFadd{The product $B_i = I_{24} \times D_i$ is the desired ball giving a destabilization.  Hence, }\DIFaddend the destabilization is achieved by setting

\begin{align*}
&X_1' = X_1 - N(I\DIFdelbegin \DIFdel{_{12} }\DIFdelend \DIFaddbegin \DIFadd{_{24} }\DIFaddend \times D_i) \\
&X_2' = X_2\\
&X_3' = X_3 \cup N(I\DIFdelbegin \DIFdel{_{12} }\DIFdelend \DIFaddbegin \DIFadd{_{24} }\DIFaddend \times D_i)
\end{align*}
...
Note that moving \DIFdelbegin \DIFdel{$\bigcup_{i=1}^g N(I_{12} \times D_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$\bigcup_{i=1}^g N(I_{24} \times D_i)$ }\DIFaddend to the $X_2'$ region rather than the $X_3'$ region would give the same topological result.

There are $g$ such destabilizations, so to get the final result we must find disjoint $D_1 \cdots D_g$ such that \DIFdelbegin \DIFdel{$B_i = I_{12} \times D_i$, and setting
}\DIFdelend \DIFaddbegin \DIFadd{$B_i = I_{24} \times D_i$, and set
}\DIFaddend \begin{align*}
&X_1' = X_1 - \bigcup_{i=1}^g N(I\DIFdelbegin \DIFdel{_{12} }\DIFdelend \DIFaddbegin \DIFadd{_{24} }\DIFaddend \times D_i) \\
&X_2' = X_2\\
&X_3' = X_3 \cup \bigcup_{i=1}^g N(I\DIFdelbegin \DIFdel{_{12} }\DIFdelend \DIFaddbegin \DIFadd{_{24} }\DIFaddend \times D_i)
\end{align*}

In the result of destabilization, \DIFdelbegin \DIFdel{$X_3' \cap M_x = B_{12} \cup \bigcup_{i=1}^g N(I_{12} \times D_i)$}\DIFdelend \DIFaddbegin \DIFadd{$X_3' \cap M_x = B_{24} \cup \bigcup_{i=1}^g N(I_{24} \times D_i)$}\DIFaddend .
This is a topological ball as can be seen in Figure \ref{getU} (the union of \DIFdelbegin \DIFdel{$B_{12}$ }\DIFdelend \DIFaddbegin \DIFadd{$B_{24}$ }\DIFaddend and regular neighborhoods of the disks in the figure).
$X_1' \cap M_x$ consists of the complement of the disks $D_i$ in $H_1$, so it is also a topological ball.  
\DIFdelbegin \DIFdel{Thus, setting }\DIFdelend \DIFaddbegin \DIFadd{We can then set  }\DIFaddend $U=X_1' \cap M_x$ and $V = X_3' \cap M_x$\DIFdelbegin \DIFdel{shows that }\DIFdelend \DIFaddbegin \DIFadd{.  The boundary $U \cap V$ consists of four disks, two of which are shown in Figure \ref{fig_uvboundary}.  Figure \ref{fig_splittinguv} shows that by performing an isotopy, this is in fact the same split $U \cap V$ as in Figure \ref{fig_disjointballs}.
Thus, }\DIFaddend we have obtained the same genus $3g+1$ trisection as constructed in Section \ref{sec_mainthm}.
\DIFaddbegin 

\begin{figure}[h]
\centering
\includegraphics[height=1.6in]{UVboundary.png}
\caption{\DIFaddFL{Two of four disks of the intersection $U \cap V$ are shown in purple.
}}
...
\caption{\DIFaddFL{Each of the four teal regions is a topological ball meeting the dark region $U$ in a disk.  Thus, $U$ can be isotoped to the union of the dark and teal regions.
}}
