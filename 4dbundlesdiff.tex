...
%DIF DEL 4dbundlesinitial.tex   Mon Dec 23 13:05:42 2019
%DIF ADD 4dbundles.tex          Mon Dec 23 15:55:56 2019
...
The triple intersection $X_1 \cap X_2 \cap X_3$ is necessarily a closed surface, and \DIFdelbegin \DIFdel{in fact }\DIFdelend \DIFaddbegin \DIFadd{by the first two conditions }\DIFaddend it induces a Heegaard splitting on each \DIFdelbegin \DIFdel{$X_i$.
To see this, note that $X$ has no boundary so }\begin{displaymath}\DIFdel{\del X_1 = X_1 \cap \overline{(X - X_1)} = X_1 \cap (X_2 \cup X_3) = (X_1 \cap X_2) \cup (X_1 \cap X_3)}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Since $X_1 \cap X_2$ and $X_1 \cap X_3$ are both 3--dimensional handlebodies, this induces a Heegaard splitting of $\del X_1$.
As the $X_i$ have disjoint interiors, $X_1 \cap X_2 \cap X_3 = \del(X_1 \cap X_2)$ is precisely the closed surface inducing the Heegaard surface.
}\DIFdelend \DIFaddbegin \DIFadd{$\del X_i$.
}\DIFaddend Let $g$ denote the genus of the trisection surface $X_1 \cap X_2 \cap X_3$\DIFdelbegin \DIFdel{, which by the above argument induces a Heegaard splitting of each $\del X_i$}\DIFdelend .
...
\begin{align*}
&X_1 = (R_1 \cup N_{23}) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{12} \cup \mathring N_{13}) \\
&X_2 = (R_2 \cup N_{13}) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{12} \cup \mathring N_{23}) \\
&X_3 = (R_3 \cup N_{12}) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{13} \cup \mathring N_{23})
\end{align*}
...
\begin{itemize}
\item A copy of \DIFdelbegin \DIFdel{$I_{12} \times (\Sigma - B_{12} \cap \Sigma)$ }\DIFdelend \DIFaddbegin \DIFadd{$I_{12} \times (\Sigma - B_{12})$ }\DIFaddend coming from the horizontal segment between $R_1$ and $R_2$.
...
$B_a$ intersects \DIFdelbegin \DIFdel{the $I_{12} \times \Sigma - (B_{12} \cap \Sigma)$ }\DIFdelend \DIFaddbegin \DIFadd{each of the $I_{12} \times (\Sigma - B_{12})$ }\DIFaddend component and the $H$ component of $X_1 \cap X_2$ in a 2--dimensional disk\DIFdelbegin \DIFdel{each}\DIFdelend .
The same is true of $B_b$, so both $B_a$ and $B_b$ can be thought of as 3--dimensional 1--handles connecting the \DIFdelbegin \DIFdel{$I_{12} \times (\Sigma - B_{12} \cap \Sigma)$ }\DIFdelend \DIFaddbegin \DIFadd{$I_{12} \times (\Sigma - B_{12})$ }\DIFaddend and $H$ components.
...
\begin{align*}
&X_1 = (R_1 \cup R_U \cup N_{23a} \cup N_{23b}) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{13}) \\
&X_2 = (R_2 \cup N_{13}) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{23a} \cup \mathring N_{23b}) \\
&X_3 = (R_3 \cup R_V) \DIFdelbegin \DIFdel{\backslash }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend (\mathring N_{13} \cup \mathring N_{23a} \cup \mathring N_{23b})
\end{align*}
...
\begin{itemize}
\item \DIFdelbegin \DIFdel{$I_{13} \times (\Sigma - \Sigma \cap B_{13})$ }\DIFdelend \DIFaddbegin \DIFadd{$I_{13} \times (\Sigma - B_{13})$ }\DIFaddend coming from the horizontal segment between $R_1$ and $R_3$.
This is the product of an interval and a punctured genus $g$ surface, so it is a genus $2g$ handlebody.

\item A copy of $U$ (a three dimensional ball) lying at each of the two vertical \DIFdelbegin \DIFdel{boundaries }\DIFdelend \DIFaddbegin \DIFadd{segments }\DIFaddend between $R_3$ and $R_4$.
...
$I_{23a} \times B^2$ and $I_{23b} \times B^2$ each intersect one copy of $U$ in a disk, and \DIFdelbegin \DIFdel{$I_{13} \times (\Sigma - \Sigma \cap B_{13})$ }\DIFdelend \DIFaddbegin \DIFadd{$I_{13} \times (\Sigma - B_{13})$ }\DIFaddend in another disk.
...
\begin{itemize}
\item \DIFdelbegin \DIFdel{$I_{23a} \times (\Sigma - \Sigma \cap B_{23a})$ and $I_{23b} \times (\Sigma - \Sigma \cap B_{23b})$ }\DIFdelend \DIFaddbegin \DIFadd{$I_{23a} \times (\Sigma - B_{23a})$ and $I_{23b} \times (\Sigma - B_{23b})$ }\DIFaddend coming from the horizontal segments between $R_2$ and $R_3$.
...
$$\DIFaddbegin \left[ \DIFaddend I_{24} \times \Sigma_{0,g+1} \DIFaddbegin \right] \DIFaddend \cup \DIFaddbegin \left[ \DIFaddend I_{23a} \times (\Sigma - \DIFdelbegin \DIFdel{\Sigma \cap }\DIFdelend B_{23a}) \DIFaddbegin \right] \DIFaddend \cup \DIFaddbegin \left[ \DIFaddend I_{23b} \times (\Sigma - \DIFdelbegin \DIFdel{\Sigma \cap }\DIFdelend B_{23b}) \DIFaddbegin \right]\DIFaddend $$ deformation retracts onto $$\DIFaddbegin \left[ \DIFaddend I_{24} \times \Sigma_{0,g+1} \DIFaddbegin \right] \DIFaddend \cup \DIFaddbegin \left[ \DIFaddend \{2/5\} \times (\Sigma - \DIFdelbegin \DIFdel{\Sigma \cap }\DIFdelend B_{23a}) \DIFaddbegin \right] \DIFaddend \cup \DIFaddbegin \left[ \DIFaddend \{3/5\} \times (\Sigma - \DIFdelbegin \DIFdel{\Sigma \cap }\DIFdelend B_{23b}) \DIFaddbegin \right]\DIFaddend $$ so it can be built from $I_{24} \times \Sigma_{0,g+1}$ by attaching $g$ 1--handles at each end of $I_{24} \times \Sigma_{0,g+1}$, giving a genus $3g$ handlebody.
...
Consider the rectangular \DIFdelbegin \DIFdel{regionshown }\DIFdelend \DIFaddbegin \DIFadd{regions shown }\DIFaddend in in Figure \ref{sectionbreakdown}.
...
\DIFdelbegin \DIFdel{The top of the boundary intersects }\DIFdelend \DIFaddbegin \DIFadd{$\del B_{23}$ decomposes into two disks, one intersecting }\DIFaddend $R_2$ \DIFdelbegin \DIFdel{while the bottom intersects }\DIFdelend \DIFaddbegin \DIFadd{and one intersecting }\DIFaddend $R_3$.
The meridian circle $\del B_{23} \cap \Sigma$ lies in the triple intersection.
Similarly if $t$ lies in the interior of \DIFdelbegin \DIFdel{$I_{23}$ then the contribution to the triple intersection is $\del B_{12} \cap \Sigma$, }\DIFdelend \DIFaddbegin \DIFadd{$I_{12}$ then $M_t \cap (X_1 \cap X_2 \cap X_3) = \del B_{12} \cap \Sigma$, which is }\DIFaddend homeomorphic to $S^1$.
...
We call the pullback of the left point of $R_1 \cap R_2 \cap R_3$ \DIFdelbegin \DIFdel{by }\DIFdelend $\Sigma_1$, and the pullback of the right point \DIFdelbegin \DIFdel{by }\DIFdelend $\Sigma_2$.
...
The $B_{12} \cap \Sigma_i$ punctures of the two surfaces are connected by the open tube in \DIFdelbegin \DIFdel{$I_{12} \cap M$}\DIFdelend \DIFaddbegin \DIFadd{$I_{12} \times M$}\DIFaddend .
...
That is, \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{if we view the surface as the boundary, minus some disks, of a standardly embedded handlebody in $\R^3$, then a }\DIFaddend simple closed curve bounds a disk in \DIFdelbegin \DIFdel{$\R^3$ in the embedding }\DIFdelend \DIFaddbegin \DIFadd{that handlebody }\DIFaddend if and only if it bounds a disk in $H_1$ when considered to be a (punctured) Heegaard surface for $M$.
...
It remains to draw the $\alpha$ \DIFdelbegin \DIFdel{curve }\DIFdelend \DIFaddbegin \DIFadd{curves }\DIFaddend bounding disks in the $I_{12} \times (\Sigma - B_{12} \cap \Sigma)$ section of $X_1 \cap X_2$.
...
This is true regardless of the choice of $c$, so in fact choosing \DIFaddbegin \DIFadd{a nonseparating collection of }\DIFaddend $2g$ such arcs \DIFdelbegin \DIFdel{, nonparallel }\DIFdelend in $\Sigma - B_{12} \cap \Sigma$ \DIFdelbegin \DIFdel{, }\DIFdelend will result in $2g$ such $\alpha$ curves.
...
The diagram could be completed by extending to a \DIFdelbeginFL \DIFdelFL{set }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{nonseparating collection }\DIFaddendFL of four \DIFdelbeginFL \DIFdelFL{nonisotopic }\DIFdelendFL disjoint symmetric $\alpha$ curves.}
\label{fig_coresubsurfacediagram_embedded}
\end{figure}

We now comment on the other possible cases.
In \DIFdelbegin \DIFdel{Diagram }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{sectionbreakdown}, the left vertical line segment pulls back under $F$ to a copy of $H_1$ in the spine, while the right pulls back to a copy of $H_2$.
The resulting trisection diagram therefore \DIFdelbegin \DIFdel{had }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend a cut system for $H_1$ on $\Sigma_1$, and a (reflected) cut system for $H_2$ on $\Sigma_2$.
The same method works as long as the cut systems are chosen to match with the vertical line segment.
If the left vertical line segment \DIFdelbegin \DIFdel{corresponded }\DIFdelend \DIFaddbegin \DIFadd{corresponds }\DIFaddend to $H_2$, we \DIFdelbegin \DIFdel{would have drawn }\DIFdelend \DIFaddbegin \DIFadd{draw }\DIFaddend a cut system for $H_2$ on $\Sigma_1$.
Similarly if the right vertical segment \DIFdelbegin \DIFdel{corresponded }\DIFdelend \DIFaddbegin \DIFadd{corresponds }\DIFaddend to $H_1$, we \DIFdelbegin \DIFdel{would have drawn }\DIFdelend \DIFaddbegin \DIFadd{draw }\DIFaddend a reflected cut system for $H_1$ on $\Sigma_2$.
...
\DIFdelbegin \DIFdel{There is }\DIFdelend \DIFaddbegin \DIFadd{The product $[1/4, 3/4] \times M$ induces }\DIFaddend a natural identification between \DIFdelbegin \DIFdel{these three copies of $M$, and this induces }\DIFdelend \DIFaddbegin \DIFadd{$M_{1/4}$, $M_{1/2}$, and $M_{3/4}$, and restricting to $\Sigma$ gives }\DIFaddend a natural identification between the three $\Sigma_i$.
\DIFaddbegin \DIFadd{We denote these identifications as $\eta_{1 \to 2}\colon \Sigma_1 \to \Sigma_2$, $\eta_{1 \to 3}\colon \Sigma_1 \to \Sigma_3$, and $\eta_{2 \to 3}\colon \Sigma_2 \to \Sigma_3$.
...
\DIFadd{Note that we can choose the punctures of $\Sigma_1$ and $\Sigma_3$ that are tubed together in a way that ensures that $\eta_{3 \to 1}$ identifies them.
...
\DIFaddend Embed $\Sigma_1$, $\Sigma_2$ and $\Sigma_3$ into $\R^3$ along three axes $\theta_1,\theta_2$, $\theta_3$ evenly distributed in $\theta$ as in Figure \ref{embeddingsigmas}.
We embed such that that rotating $\Sigma_1$ by $2\pi/3$ and then reflecting induces \DIFdelbegin \DIFdel{the natural identification between $\Sigma_1$ and $\Sigma_2$, while }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{1 \to 2}$.
Similarly }\DIFaddend rotating $\Sigma_1$ by $4\pi/3$ (\DIFaddbegin \DIFadd{with }\DIFaddend no reflection) induces \DIFdelbegin \DIFdel{the natural identification between $\Sigma_1$ and $\Sigma_3$.
Moreover, }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{1 \to 3}$.
We }\DIFaddend choose the embedding \DIFaddbegin \DIFadd{of $\Sigma_1$ }\DIFaddend so that a cut system of $H_1$ \DIFdelbegin \DIFdel{drawn on each $\Sigma_i$ }\DIFdelend coincides with an obvious system of meridians\DIFaddbegin \DIFadd{, and the simple nature of $\eta_{1 \to 2}$ and $\eta_{1 \to 3}$ ensures that this is true for our embeddings of $\Sigma_2$ and $\Sigma_3$ as well}\DIFaddend .
...
The curves drawn here will not \DIFaddbegin \DIFadd{in general }\DIFaddend be symmetric.
...
\DIFdelbegin \DIFdel{However, the monodromy is applied in this region, and neither $\Sigma_1$ nor $\Sigma_3$ was reflected to draw our diagram.
}\DIFdelend The boundary of the disk $I_{23} \times c$ consists of $c \subset \Sigma_3$, \DIFdelbegin \DIFdel{$\varphi(c) \subset \Sigma_1$}\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(c) \subset \Sigma_1$}\DIFaddend , and two arcs in the tube between $\Sigma_1$ and $\Sigma_3$ connecting the corresponding ends of $c$ and \DIFdelbegin \DIFdel{$\varphi(c)$}\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(c)$}\DIFaddend .
...
\item Draw $2g$ \DIFdelbegin \DIFdel{disjoint nonisotopic }\DIFdelend \DIFaddbegin \DIFadd{nonseparating disjoint }\DIFaddend $\alpha$ curves symmetric about $\theta_{12}$ and $2g$ \DIFdelbegin \DIFdel{disjoint nonisotopic }\DIFdelend \DIFaddbegin \DIFadd{nonseparating disjoint }\DIFaddend $\beta$ curves symmetric about $\theta_{23}$.
...
\item Let $c_1,\cdots,c_{2g}$ be a \DIFdelbegin \DIFdel{set of disjoint nonparallel }\DIFdelend \DIFaddbegin \DIFadd{nonseparating collection of disjoint }\DIFaddend arcs in $\Sigma_3$ with their boundaries lying on the puncture \DIFdelbegin \DIFdel{of $\Sigma_3$ }\DIFdelend \DIFaddbegin \DIFadd{$\{3/4\} \times \del B_{23} \subset \Sigma_3$ }\DIFaddend that is tubed to \DIFdelbegin \DIFdel{$\Sigma_1$.
Then $\varphi(c_1),\cdots,\varphi(c_{2g})$ is a set of disjoint nonparallel }\DIFdelend \DIFaddbegin \DIFadd{the puncture $\{1/4\} \times \del B_{23} \subset \Sigma_1$.
Recall that these two punctures are identified by $\eta_{3 \to 1}$.
Then $\eta_{3 \to 1}(c_1),\cdots,\eta_{3 \to 1}(c_{2g})$ is a nonseparating collection of disjoint }\DIFaddend arcs in $\Sigma_1$ with their boundaries lying on the puncture of $\Sigma_1$ that is tubed to $\Sigma_3$.
Draw $2g$ \DIFdelbegin \DIFdel{nonisotopic }\DIFdelend \DIFaddbegin \DIFadd{nonseparating disjoint }\DIFaddend $\gamma$ curves by connecting up each $c_i$ with \DIFdelbegin \DIFdel{$\varphi(c_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(c_i)$ }\DIFaddend using two arcs in the tube between $\Sigma_1$ and $\Sigma_3$.
If $a,b$ are the two endpoints of $c_i$, we connect $c_i$ and \DIFdelbegin \DIFdel{$\varphi(c_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(c_i)$ }\DIFaddend such that $a$ is connected to \DIFdelbegin \DIFdel{$\varphi(a)$ }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(a)$ }\DIFaddend and $b$ to \DIFdelbegin \DIFdel{$\varphi(b)$}\DIFdelend \DIFaddbegin \DIFadd{$\eta_{3 \to 1}(b)$}\DIFaddend .
...
Restricting the bottom two $\gamma$ curves to $\Sigma_3$ gives a \DIFdelbeginFL \DIFdelFL{complicated }\DIFdelendFL pair of \DIFaddbeginFL \DIFaddFL{trivial looking }\DIFaddendFL arcs that turn into \DIFdelbeginFL \DIFdelFL{trivial looking }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{complicated }\DIFaddendFL arcs when \DIFdelbeginFL \DIFdelFL{$\varphi$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\eta_{3 \to 1}$ }\DIFaddendFL is applied to the surface.
...
To begin, draw the triple intersection surface for this diagram as four \DIFaddbegin \DIFadd{twice punctured }\DIFaddend copies of $\Sigma$ connected by tubes.
...
\DIFdelbegin \DIFdel{Draw them }\DIFdelend \DIFaddbegin \DIFadd{In our construction $\Sigma_1, \Sigma_2, \Sigma_3$, and $\Sigma_4$ correspond to the copies of $\Sigma$ lying in $M_{1/5}, M_{2/5}, M_{3/5}$, and $M_{4/5}$ respectively, and can be pictured as lying at the triple intersection corners in Figure \ref{preservebreakdown}.
...
\DIFadd{Draw the $\Sigma_i$ }\DIFaddend in the plane at evenly distributed $\theta$ values $\theta_1$, $\theta_2$, $\theta_3$, and $\theta_4$, with all holes of $\Sigma_i$ lying along the $\theta_i$ axis.
...
\DIFaddbegin \DIFadd{Thus, $\eta_{1 \to 3}$ is the result rotating $\Sigma_1$ around the center of the diagram from $\theta_1$ to $\theta_3$.
$\eta_{1 \to 2}$ (resp. $\eta_{1 \to 4}$) is the result of rotating $\Sigma_1$ from $\theta_1$ to $\theta_2$ (resp. $\theta_4$) and then reflecting.
}\DIFaddend This ensures that the curves bounding product disks in the copies of $I_{ij} \times (\Sigma - D^2)$ are symmetric except for \DIFaddbegin \DIFadd{the curves bounding disks in }\DIFaddend $I_{13} \times (\Sigma - D^2)$\DIFdelbegin \DIFdel{where the monodromy map occurs.
...
\DIFaddend Again let $\theta_{ij}$ be the angle halfway between $\theta_i$ and $\theta_j$, lying in the smaller ($< \pi$) angular region between them.
...
\item Draw $2g$ \DIFdelbegin \DIFdel{disjoint nonisotopic }\DIFdelend \DIFaddbegin \DIFadd{nonseparating disjoint }\DIFaddend $\gamma$ curves symmetric about $\theta_{12}$ and avoiding $\Sigma_3$ and $\Sigma_4$, and another $2g$ $\gamma$ curves symmetric about $\theta_{34}$ and avoiding $\Sigma_1$ and $\Sigma_2$.
\item Draw a single $\gamma$ curve as a meridian of the tube connecting $\Sigma_1$ and $\Sigma_4$.
\item Draw $2g$ \DIFdelbegin \DIFdel{disjoint nonisotopic }\DIFdelend \DIFaddbegin \DIFadd{nonseparating disjoint }\DIFaddend $\alpha$ curves symmetric about $\theta_{23}$ and avoiding $\Sigma_1$ and $\Sigma_4$.
...
\item Let $c_1,\cdots,c_{2g}$ be a \DIFdelbegin \DIFdel{set of disjoint nonparallel arcs in $\Sigma_1$ }\DIFdelend \DIFaddbegin \DIFadd{nonseparating collection of disjoint arcs in $\Sigma_4$ }\DIFaddend with their boundaries lying on the puncture of \DIFdelbegin \DIFdel{$\Sigma_1$ }\DIFdelend \DIFaddbegin \DIFadd{$\Sigma_4$ }\DIFaddend that is tubed to \DIFdelbegin \DIFdel{$\Sigma_4$}\DIFdelend \DIFaddbegin \DIFadd{$\Sigma_1$}\DIFaddend .
Then connect each $c_i$ to its image \DIFdelbegin \DIFdel{$\varphi(c_i)$ in $\Sigma_4$ }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{4 \to 1}(c_i)$ in $\Sigma_1$ }\DIFaddend with the opposite orientation.
Draw these $2g$ curves as $\beta$ curves.
Note that the \DIFdelbegin \DIFdel{$\varphi(c_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$\eta_{4 \to 1}(c_i)$ }\DIFaddend will need to be drawn reflected in $\theta$ because \DIFaddbegin \DIFadd{$\eta_{1 \to 4}$ reflected the embedding of }\DIFaddend $\Sigma_4$ \DIFdelbegin \DIFdel{was reflected .
If $\varphi$ }\DIFdelend \DIFaddbegin \DIFadd{relative to $\Sigma_1$.
If $\eta_{4 \to 1}$ }\DIFaddend acts trivially on $\Sigma$ these curves will look symmetric about $\theta_{41}$.
\end{enumerate}
...
See Figure \ref{destabpartial} for a partial \DIFdelbegin \DIFdel{Heegaard }\DIFdelend \DIFaddbegin \DIFadd{trisection }\DIFaddend diagram in the case where $g=2$.
...
The destabilizations are therefore obtained by ``compressing'' $X_1$ along $B_i$\DIFdelbegin \DIFdel{-- if }\DIFdelend \DIFaddbegin \DIFadd{.
If }\DIFaddend $N(B_i)$ is a regular neighborhood of $B_i$ in $X_1$, we replace $X_1$ with $X_1 - N(B_i)$ and replace $X_2$ with $X_2 \cup N(B_i)$ (choosing $X_3$ instead of $X_2$ here is topologically equivalent).
Each compression \DIFdelbegin \DIFdel{decreasing }\DIFdelend \DIFaddbegin \DIFadd{decreases }\DIFaddend the genus of $X_1$ by 1.
...
\DIFdelbegin \DIFdel{$M_x \cap \alpha_i$ is an arc in $(H_1 - B_{12}) \cap H_2$ while $M_x \cap \beta_i$ is an arc in $B_{12} \cap (H_1 - B_{12})$.
The }\DIFdelend \DIFaddbegin \DIFadd{$M_x \cap \alpha_i = M_x \cap \beta_i$ is a pair of two points.
...
In this way, we can obtain disks $D_{\alpha_i}$ and $D_{\beta_i}$ bounded by $\alpha_i = \beta_i$ by taking the }\DIFaddend union of these \DIFdelbegin \DIFdel{two arcs }\DIFdelend \DIFaddbegin \DIFadd{arcs for all $x \in I_{12}$.
...
\DIFadd{We now need to find the ball $B_i \subset X_1$ bounded by $D_{\alpha_i} \cup D_{\alpha_i}$.
Looking back at a single $M_x$, the union of the arc from $D_{\alpha_i}$ and the arc from $D_{\beta_i}$ }\DIFaddend is a simple closed curve in $\del (H_1 - B_{12})$, and this curve bounds a disk $D_i$ in $H_1 - B_{12}$.
See Figure \ref{getU}.
...
Baykur and Saeki found genus $3g+1$ trisections in the case of Heegaard splitting preserving monodromy using simplified broken Lefschetz fibrations \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BaykurSaeki1}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
